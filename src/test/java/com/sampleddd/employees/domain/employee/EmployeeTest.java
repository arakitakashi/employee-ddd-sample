...
class EmployeeTest {
    ...
    @CsvSource(delimiter = '|', textBlock = """
        # ID | FIRST NAME | LAST NAME | MESSAGE                      | TESTNAME
          -1 |            |    YAMADA | employee ID must be positive | IDがマイナス
           ...
           1 |            |    YAMADA | firstName must not be blank | 名前がnull
           1 |         '' |    YAMADA | firstName must not be blank | 名前が空
           1 |       '  ' |    YAMADA | firstName must not be blank | 名前がスペースのみ
           1 |       TARO |           | lastName must not be blank  | 名字がnull
           1 |       TARO |        '' | lastName must not be blank  | 名字が空
    ...
    void IDの値が10桁以内ではない場合不正を検出する() throws Exception {
        // assert
        assertThatThrownBy(
            () -> new Employee(MAX_LENGTH_EXCEEDED_ID, VALID_FIRST_NAME, VALID_LAST_NAME))
            .isInstanceOf(IllegalArgumentException.class)
    ...
    void 名の値が100桁以内ではない場合不正を検出する() throws Exception {
        // assert
        assertThatThrownBy(
            () -> new Employee(VALID_ID, MAX_LENGTH_EXCEEDED_FIRST_NAME, VALID_LAST_NAME))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("firstName must not exceed the maximum length");
    }
    ...
    void 姓の値が100桁以内ではない場合不正を検出する() throws Exception {
        // assert
        assertThatThrownBy(
            () -> new Employee(VALID_ID, VALID_FIRST_NAME, MAX_LENGTH_EXCEEDED_LAST_NAME))
            .isInstanceOf(IllegalArgumentException.class)
            .hasMessage("lastName must not exceed the maximum length");
    }
}
